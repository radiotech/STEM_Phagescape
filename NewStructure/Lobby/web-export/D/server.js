...
function Env() {
    ...
    e.update = function() {
        ...
        if (e.tick % 25 == 0) {
            /*
             * println(0,true); println("[SERVER CONSOLE]"); println("Upload: "+Math.ceil(upload*0.001)+" kbps");upload = 0; println("Download: "+Math.ceil(download*0.001)+" kbps");download = 0; println("Users: "+Object.keys(e.users).length); println("Worlds: "+e.worlds.length); if(outputText.length > 0){ println(""); println("[SERVER OUTPUT]"); outputText.forEach(function(text,i){ println(text); if(e.tick > outputTick[i]){ outputText.splice(i,1); outputTick.splice(i,1); }
             * 
             * }); }
             */
            /*
