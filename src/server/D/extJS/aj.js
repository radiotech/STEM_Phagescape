...
var getConfig = function(callback){
	$.getJSON("https://spreadsheets.google.com/feeds/list/170ikQITKF3bz-EGAPZjS6gun3FVdCyLRcEVtblLWx4Y/od6/public/full?alt=json",function(data){
		callback(JSON.parse(data.feed.entry[0].title.$t));
	});
}
		...
		case "tip": //TIP, id, dir, x, y, text, fade
			if($(".splash-bg").length == 0) {
				...
				if (x != lastx || y != lasty) {
					...
					if($tip.data('dead') == undefined){
						$tip.data('dead',true);
					}
					
					if ($tip.data('dead')) {
						console.log("Born "+args[5]);
						$tip.data('dead', false);
